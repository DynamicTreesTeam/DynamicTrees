...
import com.dtteam.dynamictrees.model.BakedQuadMover;
...
public class FallingTreeEntityModelNF extends FallingTreeEntityModel {
    ...
    private static List<BakedQuad> collectBranchQuads(BlockStateModel model, BlockState state, int[] connections,
        ...
        BasicBranchBlockBakedModel branch = unwrapBranch(model);
        if (branch != null) {
            return branch.collectQuads(state, connections, forceRingDir);
        }
        return collectVanillaQuads(model, random);
    }

    @Nullable
    private static BasicBranchBlockBakedModel unwrapBranch(BlockStateModel model) {
        BlockStateModel current = model;
        for (int i = 0; i < 8 && current != null; i++) {
            if (current instanceof BreakingOverlayModel overlay) {
                current = overlay.inner();
                continue;
            }
            if (current instanceof BasicBranchBlockBakedModel baked) {
                return baked;
            }
            break;
        }
