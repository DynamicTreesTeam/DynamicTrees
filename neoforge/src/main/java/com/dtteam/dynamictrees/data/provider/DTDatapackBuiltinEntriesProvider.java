...
public class DTDatapackBuiltinEntriesProvider extends DatapackBuiltinEntriesProvider implements DTDataProvider {
    ...
    private static void replaceNyliumFungiFeatures(HolderLookup.Provider vanillaProvider, BootstrapContext<ConfiguredFeature<?, ?>> context) {
        TreeRegistry.findSpecies(DynamicTrees.CRIMSON).getSapling().ifPresent(crimsonSapling ->
                TreeRegistry.findSpecies(DynamicTrees.WARPED).getSapling().ifPresent(warpedSapling -> {
