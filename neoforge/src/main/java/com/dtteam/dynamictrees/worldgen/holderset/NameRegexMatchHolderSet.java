...
public class NameRegexMatchHolderSet<T> extends RegexMatchHolderSet<T> {
    ...
    public static class Type implements HolderSetType {
        ...
        public <T> MapCodec<? extends ICustomHolderSet<T>> makeCodec(ResourceKey<? extends Registry<T>> resourceKey, Codec<Holder<T>> codec, boolean b) {
            return RegexMatchHolderSet.mapCodec(resourceKey, NameRegexMatchHolderSet::new);
        }
        ...
        public <T> StreamCodec<RegistryFriendlyByteBuf, ? extends ICustomHolderSet<T>> makeStreamCodec(ResourceKey<? extends Registry<T>> resourceKey) {
            return ByteBufCodecs.fromCodecWithRegistriesTrusted(RegexMatchHolderSet.mapCodec(resourceKey, NameRegexMatchHolderSet::new).codec());
        }
    }
    ...
    public HolderSetType type() {
        return NeoForgeRegistryLoader.NAME_REGEX_MATCH_HOLDER_SET_TYPE.get();
    }
}