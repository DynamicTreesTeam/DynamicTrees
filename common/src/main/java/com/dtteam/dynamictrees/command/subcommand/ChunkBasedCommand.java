...
public abstract class ChunkBasedCommand extends SubCommand {
    ...
    private ChunkPos getChunkPos(final CommandSourceStack source) {
        return ChunkPos.containing(BlockPos.containing(source.getPosition().x, source.getPosition().y, source.getPosition().z));
    }

    @Override
    public ArgumentBuilder<CommandSourceStack, ?> registerArgument() {
        return blockPosArgument().executes(context -> executesSuccess(() -> this.processChunk(context.getSource(),
                        context.getSource().getLevel(), ChunkPos.containing(blockPosArgument(context)), DEFAULT_RADIUS)))
                .then(Commands.argument(RADIUS, IntegerArgumentType.integer(1))
                        .executes(context -> executesSuccess(() -> this.processChunk(context.getSource(),
                                context.getSource().getLevel(), ChunkPos.containing(blockPosArgument(context)), intArgument(context, RADIUS)))));
    }
