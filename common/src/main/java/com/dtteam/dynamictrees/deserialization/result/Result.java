...
public interface Result<T, I> {
    ...
    /**
     * Executes the specified {@code consumer} with the deserialised value if this is a successful result, otherwise
     * throws a {@link DeserializationException} with the result's error as the detail message. Then executes the
    ...
    default Result<T, I> ifSuccessOrElseThrow(Consumer<T> consumer, Consumer<String> warningConsumer)
        ...
        if (this.success()) {
            ...
            throw new DeserializationException(this.getError());
        }
        this.getWarnings().forEach(warningConsumer);
        return this;
    }
    ...
    /**
     * Returns the deserialised value if this is a successful result, or throws a {@link DeserializationException} if
     * not.
     *
     * @return the deserialised value
     ...
     * DeserializationException}, whose message should be used as the mapped result's error if thrown.
     * <p>
     ...
     */
    @FunctionalInterface
    interface SimpleMapper<T, V> extends ThrowableFunction<T, V, DeserializationException> {
        /**
     ...
     * application can also throw a {@link DeserializationException}, whose message should be used as the mapped
     ...
     */
    @FunctionalInterface
    interface Mapper<T, V> extends ThrowableBiFunction<T, Consumer<String>, V, DeserializationException> {
    }
