...
public class LeafLitterGenFeature extends GenFeature {
    ...
    protected boolean postGenerate(GenFeatureConfiguration configuration, PostGenerationContext context) {
        ...
        for (int j = 0; j < qty; j++) {
            ...
            if (placePos != null) {
                placeLeafLitter(level, placePos, leafLitter, random, Block.UPDATE_CLIENTS, maxAmount);
            }
        }
        return true;
    }
    ...
    protected boolean postGrow(GenFeatureConfiguration configuration, PostGrowContext context) {
        ...
        if (placePos != null) {
            ...
            if (surroundingLitter > configuration.get(MAX_LITTER_AROUND)) {
                return false;
            }
    ...
    private static int countLitterAround(BlockPos placePos, LevelAccessor level, Block leafLitter) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (CoordUtils.Surround sur : CoordUtils.Surround.values()) {
                ...
                if (level.getBlockState(side).is(leafLitter)) {
                    surroundingLitter++;
                }
            }
        }
    ...
    private static void placeLeafLitter(LevelAccessor level, BlockPos placePos, Block leafLitter, RandomSource rand, int flags, int maxAmount) {
        BlockState state = leafLitter.defaultBlockState();
        if (state.hasProperty(LeafLitterBlock.FACING)) {
            state = state.setValue(LeafLitterBlock.FACING, CoordUtils.getRandomDir(rand));
        }
