...
public class CreakingHeartGenFeature extends GenFeature {
    ...
    public boolean shouldApply(Species species, GenFeatureConfiguration configuration) {
        return species.getFamily() instanceof CreakingHeartFamily chf && chf.getResinBranch().isPresent();
    }
    ...
    protected boolean postGenerate(GenFeatureConfiguration configuration, PostGenerationContext context) {
        ...
        if (TreeHelper.getRadius(context.level(), context.pos().above()) < configuration.get(FRUITING_RADIUS)) {
            return false;
        }
        ...
        for (int i = 1; i < maxHeight; i++) {
            ...
            if (isRadiusJustRight(testState, configuration)
                ...
                heart.setRadius(context.level(), testPos, TreeHelper.getTreePart(testState).getRadius(testState), Direction.DOWN, 3);
                return true;
            }
        }
        return false;
    ...
    private int getLowestBranchHeight(PostGenerationContext context) {
        return context.species().getGrowthLogicKit().getLowestBranchHeight(
    ...
    private boolean isRadiusJustRight(BlockState branchState, GenFeatureConfiguration configuration) {
        ...
        int radius = TreeHelper.getTreePart(branchState).getRadius(branchState);
