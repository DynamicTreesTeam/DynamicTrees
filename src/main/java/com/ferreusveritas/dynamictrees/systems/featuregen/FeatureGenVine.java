...
public class FeatureGenVine implements IPostGenFeature {
	...
	/**
	 * The vine block. Obtained from registry by name to fix JurassicCraft compat (since they replace
	 * vanilla vines). For this reason it is also a lazy value to ensure all registries have been
	 * filled by the time it is obtained (during generation, at which point all blocks will definitely
	 * be registered). 
	 */
	protected LazyValue<Block> vineBlock = LazyValue.uninitialised(() -> Block.getBlockFromName("vine"));
