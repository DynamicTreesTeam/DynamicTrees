...
public class TreeJungle extends DynamicTree {
	...
	@Override
	public boolean onTreeActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
			...
			if(heldItem.getItem() == Items.dye && heldItem.getItemDamage() == 3) {
				...
				if(branch != null && branch.getRadius(world, pos) == 8) {
					...
					if (world.isAirBlock(pos)) {
						int meta = DynamicTrees.blockFruitCocoa.onBlockPlaced(world.getWorld(), pos.getX(), pos.getY(), pos.getZ(), side.getIndex(), hitX, hitY, hitZ, 0);
						world.setBlockState(pos, new BlockAndMeta(DynamicTrees.blockFruitCocoa, meta), 2);
