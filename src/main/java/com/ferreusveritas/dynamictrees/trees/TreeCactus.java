		...
		public boolean setBlockForGeneration(World world, Species species, BlockPos pos, Direction dir, boolean careful) {
			...
			if (world.getBlockState(pos).getMaterial().isReplaceable()) {
				...
				if (dir == Direction.UP) {
					...
					if (TreeHelper.isRooty(downState) || (downState.getBlock() == defaultBranchState.getBlock() && downState.get(BlockBranchCactus.TRUNK) && downState.get(BlockBranchCactus.ORIGIN) == Direction.DOWN)) {
						trunk = true;
					}
				}
				return !world.setBlockState(pos, defaultBranchState.with(BlockBranchCactus.TRUNK, trunk).with(BlockBranchCactus.ORIGIN, dir.getOpposite()), careful ? 3 : 2);
			}
			return true;
		}
