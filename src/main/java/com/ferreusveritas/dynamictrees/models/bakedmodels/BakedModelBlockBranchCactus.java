...
public class BakedModelBlockBranchCactus implements IDynamicBakedModel {
    ...
    @Override
	public List<BakedQuad> getQuads(@Nullable BlockState blockState, @Nullable Direction side, @Nonnull Random rand) {
		List<BakedQuad> quadsList = new ArrayList<>(12);

		if (blockState != null) {
			int coreRadius = getRadius(blockState);
			int[] connections = pollConnections(coreRadius, blockState);
			...
			if (coreRadius == 4 && numConnections == 1 && blockState.get(BlockBranchCactus.ORIGIN).getAxis().isHorizontal()) {
				connections[1] = 4;
				extraUpSleeve = true;
			}
			...
			for (Direction connDir : Direction.values()) {
				...
				if (connRadius > 0 && ((connDir == Direction.UP && connRadius == 4 && extraUpSleeve) || side != connDir || connDir == Direction.DOWN)) {
					quadsList.addAll(sleeves[idx][connRadius - 4].getQuads((BlockState)blockState, side, rand));
				}
			}
			if (extraUpSleeve) {
				quadsList.addAll(sleeveTopSpikes.getQuads((BlockState)blockState, Direction.UP, rand));
			}
		}
