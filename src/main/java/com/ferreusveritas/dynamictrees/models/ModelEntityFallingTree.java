	...
	public static List<BakedQuad> generateTreeQuads(EntityFallingTree entity) {
		...
		if(destructionData.getNumBranches() > 0) {
			...
			if(exState != null) {
				...
				treeQuads.addAll(QuadManipulator.getQuads(branchModel, exState, new Vec3d(BlockPos.ORIGIN.offset(cutDir)).scale(offset), new Direction[] { cutDir }));
				...
				if(leavesClusters != null) {
					for(Entry<BlockPos, BlockState> leafLoc : leavesClusters.entrySet()) {
