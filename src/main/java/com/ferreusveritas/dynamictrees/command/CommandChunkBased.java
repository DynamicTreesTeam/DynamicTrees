...
public abstract class CommandChunkBased extends SubCommand {
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos targetPos) {
		...
		switch(args.length) {
			case 2: 
			case 3: return getTabCompletionCoordinate(args, 1, targetPos);
			case 4: return Lists.newArrayList(Integer.toString(0));
		}
	...
	public void execute(World world, ICommandSender sender, String[] args) throws CommandException {
		...
		if(args.length >= 3) {
			...
			cPos = new ChunkPos(
					(int)CommandBase.parseDouble((double)(blockpos.getX() >> 4), args[1], -1875000, 1875000, false),
					(int)CommandBase.parseDouble((double)(blockpos.getZ() >> 4), args[2], -1875000, 1875000, false)
					);
	...
	public static List<String> getTabCompletionCoordinate(String[] inputArgs, int index, @Nullable BlockPos pos) {
		...
		switch(inputArgs.length - index - 1) {
			case 0: v = pos.getX(); break;
			case 1: v = pos.getZ(); break;
