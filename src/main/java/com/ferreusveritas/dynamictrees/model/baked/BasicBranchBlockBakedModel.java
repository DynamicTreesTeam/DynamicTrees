...
public class BasicBranchBlockBakedModel extends BranchBlockBakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull Random random, @Nonnull IModelData modelData) {
        ...
        if (modelData instanceof ModelConnections) {
            final ModelConnections connectionsData = (ModelConnections) modelData;
        ...
        if (numConnections == 0 && forceRingDir != null) {
            quadsList.addAll(rings[coreRadius - 1].getQuads(state, forceRingDir, random, modelData));
        } else {
            ...
            for (Direction face : Direction.values()) {
                ...
                if (coreRadius != connections[face.get3DDataValue()]) {
                    if ((coreRingDir == null || coreRingDir != face)) {
                        quadsList.addAll(cores[coreDir][coreRadius - 1].getQuads(state, face, random, modelData));
                    } else {
                        quadsList.addAll(rings[coreRadius - 1].getQuads(state, face, random, modelData));
                    }
                }
                ...
                if (coreRadius != 8) { // Special case for r!=8.. If it's a solid block so it has no sleeves.
                    for (Direction connDir : Direction.values()) {
                        ...
                        if (connRadius > 0 && (connRadius == twigRadius.get() || face != connDir)) {
                            quadsList.addAll(sleeves[idx][connRadius - 1].getQuads(state, face, random, modelData));
                        }
                    }
                }
    ...
    public IModelData getModelData(@Nonnull BlockAndTintGetter level, @Nonnull BlockPos pos, @Nonnull BlockState state, @Nonnull IModelData modelData) {
        ...
        return new ModelConnections(((BranchBlock) block).getConnectionData(level, pos, state))
