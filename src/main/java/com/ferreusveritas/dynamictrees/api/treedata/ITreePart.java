...
public interface ITreePart {
	...
	/**
	 * Returns an {@link ICell} that provides the level of hydration to neighboring
	 * structures.
	 *
	 * @param world            read-only access to blocks
	 * @param pos              the position of the block at which to gather surrounding
	 *                         cells
	 * @param state            the state of the block we are getting the cell from
	 * @param dir              The direction of the request (opposite the direction of
	 *                         the requester)
	...
	/**
	 * The signal that is passed from the root of the tree to the tip of a branch to
	 * create growth.
	 *
	...
	/**
	 * Returns the probability that the branch logic will follow into this block as
	 * part of its path.
	 *
	 ...
	 * @return the probability weight used to determine if the growth path will take
	 * this block as a path next
	 */
	...
	/**
	 * Returns the radius of a branch, or {@code 0} if this {@link ITreePart} is not a
	 * {@link BlockBranch} instance.
	 *
	...
	/**
	 * Returns whether this node should be analyzed. Branches should always return
	 * {@code true} where leaves should always return {@code false}. Other types may vary in
	 * return depending on the implementation.
	 *
