...
/**
 * Any objects using {@link Registry} can be registered using this registry event. Gives full access to the {@link
 * Registry}, allowing things like {@link Registry#has(ResourceLocation)} to be checked. There is no need to check if
 * the {@link Registry} is locked as it should always be unlocked before calling this event.
 *
...
public final class RegistryEvent<V extends RegistryEntry<V>> extends GenericEvent<V> implements IModBusEvent {
    ...
    public RegistryEvent(final IRegistry<V> registry) {
